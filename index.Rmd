...
## Sampling strategies: *stratified* (random) sampling  
...
(the probability of selection is not the same for all elements in teh sample, and does depends on oterh factors)
...
DATA[DATA$geo1 == "GT",]$globorisk_nonlab <- DATA[DATA$geo1 == "GT",]$globorisk_nonlab*3    # EXAGGERATING GT RISK FOR CLARITY
DATA[DATA$globorisk_nonlab > 100,]$globorisk_nonlab <- 98
...
TRUEAVRISK <- round(weighted.mean(DATA1$globorisk_nonlab,DATA1$sweight,na.rm = FALSE),1)    # HYPOTHETICAL 'TRUE' POPULATION RISK
...
df <- merge(data.frame(round(prop.table(table(DATA$geo1))*100,1)), POP2016, by.x = "Var1", by.y = "geo1") %>% rename(geo1 = Var1)
df$Pop_perc <- round(df$pop/sum(df$pop)*100,1)
df 
```
]
...
psamp <- df$Freq/df$pop %>% set_names(c("EC","FS","GT","KZN","LI","MP","NC","NW","WC"))
psamp

sweight <- 1/psamp %>% set_names(c("EC","FS","GT","KZN","LI","MP","NC","NW","WC"))
sweight

sweight <- (sweight/sum(sweight)) %>% set_names(c("EC","FS","GT","KZN","LI","MP","NC","NW","WC"))
sweight 
